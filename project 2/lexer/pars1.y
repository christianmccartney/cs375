...
//              |  function
  ...
  consts        :  IDENTIFIER EQ NUMBER SEMICOLON consts { instconst($1, $3); }
                ...
                ;
  types         :  typedef SEMICOLON types      //{ $$ = nconc($1, $3); }
                |  typedef SEMICOLON            { $$ = $1; }
                ;
  typedef	:  IDENTIFIER EQ type           { insttype($1, $3); }
                ;
  type          :  LPAREN idlist RPAREN         { $$ = instenum($2); }
                |  RECORD fieldlist END         { $$ = instrec($1, $2); }
		|  POINT IDENTIFIER		{ $$ = instpoint((TOKEN)talloc(), $2); }
                ;
  simpletype    :  IDENTIFIER                   { $$ = findtype($1); }
                |  NUMBER DOTDOT NUMBER         { $$ = instdotdot($1, $2, $3); }
                ;
  simplelist    :  simpletype COMMA simplelist  { $$ = cons($1, $3); }
                |  simpletype                   { $$ = $1; }
                ;
  var           :  vars SEMICOLON var           { $$ = $3; }
                |  vars SEMICOLON               { $$ = $1; }
                ;
  vars          :  idlist COLON newtype         { instvars($1, $3); }
                ;
  newtype       :  ARRAY LBRACKET simplelist RBRACKET OF IDENTIFIER { $$ = instarray($3, $6); }
